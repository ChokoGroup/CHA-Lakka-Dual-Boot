...
then
  ...
  cp "$RUNNINGFROM/lakka1080p.nfo" /.choko/games2I.nfo
  cp "$RUNNINGFROM/lakka1080p.sh" /.choko/games2I.sh
  ...
  echo -e "Menu option assigned to [P2 I]."

  if [ -f /.choko/games2S.nfo ]
  then
    ...
    GAMES2S="$(cat /.choko/games2S.nfo)"
    echo -e "\nDo you want to delete \"$GAMES2S\" option assigned to [P2 S]?\e[m\nPress \e[1;94m[P2 START]\e[m to delete or [P1 A] to cancel."
    COUNTDOWN=10
