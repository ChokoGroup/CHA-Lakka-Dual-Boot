...
if [ -f "/.choko/Run RetroArch automatic resolution.nfo" ]
then
  rm "/.choko/Run RetroArch automatic resolution."*
  echo "Menu option \"Run RetroArch (automatic resolution)\" uninstalled (Lakka files were not deleted)."
else
  echo "\"/.choko/Run RetroArch automatic resolution.*\" not found."
fi
sleep 3
